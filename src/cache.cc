                ...
                if ((mshr_index == -1) && (MSHR.occupancy < MSHR_SIZE)) { // this is a new miss
                    ...
                    if ((mshr_index == -1) && (MSHR.occupancy == MSHR_SIZE)) { // not enough MSHR resource
                        ...
                        if (MSHR.entry[mshr_index].type == PREFETCH) {
                            ...
                            WQ.entry[index].cycle_enqueued = MSHR.entry[mshr_index].cycle_enqueued;
...
if((cache_type == IS_L1I || cache_type == IS_L1D) && reads_ready.size() == 0)
    ...
    for (uint32_t i=0; i<MAX_READ; i++) {
        ...
        if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0)) {
            ...
            if (way >= 0) { // read hit
                ...
                if (block[set][way].prefetch) {
                    pf_useful++;
                    if (block[set][way].train) {
                        if (block[set][way].pf == 1)
                            pf1_useful++;
                        else if (block[set][way].pf == 2)
                            pf2_useful++;
                        else if (block[set][way].pf == 3)
                            pf3_useful++;
                    }
                ...
                reads_available_this_cycle--;
                    ...
                    if ((mshr_index == -1) && (MSHR.occupancy < MSHR_SIZE)) { // this is a new miss
                        ...
                        if(cache_type == IS_STLB && MSHR.entry[mshr_index].type == TRANSLATION_FROM_L1D) {
                            pf_lower_level++;
                            if(RQ.entry[index].train) {
                                if(RQ.entry[index].pf == 1)
                                    pf1_lower_level++;
                                else if(RQ.entry[index].pf == 2)
                                    pf2_lower_level++;
                                else if(RQ.entry[index].pf == 3)
                                    pf3_lower_level++;
                            }
                        }
                ...
                if ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[prefetch_cpu]) && (PQ.occupancy > 0))
                {
                    ...
                    if (way >= 0) { // prefetch hit
                        ...
                        if ((mshr_index == -1) && (MSHR.occupancy < MSHR_SIZE)) { // this is a new miss
                            ...
                            if(PQ.entry[index].train) {
                                if(PQ.entry[index].pf == 1) {
                                    ++pf1_lower_level;
                                }
                                else if(PQ.entry[index].pf == 2) {
                                    ++pf2_lower_level;
                                }
                                else if(PQ.entry[index].pf == 3) {
                                    ++pf3_lower_level;
                                }
                            }
        ...
        void CACHE::fill_cache(uint32_t set, uint32_t way, PACKET *packet)
        {
            ...
            block[set][way].train = packet->train;
            block[set][way].pf = packet->pf;
        ...
        int CACHE::prefetch_line(uint64_t ip, uint64_t base_addr, uint64_t pf_addr, int pf_fill_level, uint32_t prefetch_metadata, uint8_t train = 0, uint8_t pf = 0) /*, uint64_t prefetch_id)*/		//Neelu: commented. 
        {
            //	if(cache_type == IS_L2C)
            ...
            if (PQ.occupancy < PQ.SIZE) {
                ...
                if (pf_packet.train) {
                    if (pf_packet.pf == 1) pf1_issued++;
                    if (pf_packet.pf == 2) pf2_issued++;
                    if (pf_packet.pf == 3) pf3_issued++;
                }
